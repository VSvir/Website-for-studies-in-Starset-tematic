...
class S(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if '.' in self.path:
            elems = self.path.rsplit('.', 1)
            print('elems', elems)
            if len(elems) == 2:
                ext = elems[1]
                ctype = ContentRegistry.content_type(ext)
                print('ctype', ctype)
                if ctype is not None:
                    ...
                    logging.info(f'try to read non-html {fname}')
    ...
    def do_echo(self):
        self._set_headers()
        response = self._html("echo_body", Templates.echo(self))
        fname = os.path.join(Config.STATIC_ROOT, 'echo-result.html')
